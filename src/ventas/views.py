            ...
            with transaction.atomic():
                ...
                for i, producto_id in enumerate(productos_ids):
                    if producto_id:
                        ...
                        # Procesar cantidad (Decimal directo)
                        cantidad = Decimal(cantidades[i])
                        
                        # Validar enteros para productos por unidad
                        if not producto.es_por_peso:
                            # Verificar si tiene decimales (ej: 1.5 % 1 = 0.5)
                            if cantidad % 1 != 0:
                                raise ValueError(f"El producto '{producto.nombre}' se vende por unidades, no admite decimales ({cantidad}).")
