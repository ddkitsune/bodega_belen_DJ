...
def producto_create(request):
    ...
    if request.method == 'POST':
        try:
            ...
            cantidad = Decimal(request.POST.get('cantidad', '0.000').replace(',', '.'))
            stock_minimo = Decimal(request.POST.get('stock_minimo', '5.000').replace(',', '.'))
...
def producto_update(request, pk):
    ...
    if request.method == 'POST':
        try:
            ...
            producto.cantidad = Decimal(request.POST.get('cantidad', '0.000').replace(',', '.'))
            producto.stock_minimo = Decimal(request.POST.get('stock_minimo', '5.000').replace(',', '.'))
...
def movimiento_create(request):
    ...
    if request.method == 'POST':
        try:
            ...
            cantidad = Decimal(request.POST.get('cantidad').replace(',', '.'))
